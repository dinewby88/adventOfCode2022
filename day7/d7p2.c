...
#define REQUIRED_SIZE_FOR_UPDATE 30000000
...
int currentBestFileSize = INT_MAX;
...
int calculateUsedSpace(directory_t* dir, int level, int sum)
{
    ...
    if (ptr)
    {
        indirectFileSizeSum += calculateUsedSpace(ptr,level+1,indirectFileSizeSum);

        while(ptr->next)
        {
            indirectFileSizeSum += calculateUsedSpace(ptr->next,level+1,indirectFileSizeSum);
            ptr = ptr->next;
        }
    }

    //printf("indirect file size = %d\n", indirectFileSizeSum);
    int totalSize = directFilesSizeSum+indirectFileSizeSum;
    return totalSize;
}
...
int calculateFolders(directory_t* dir, int level, int sum)
{
    ...
    if ((totalSize > (REQUIRED_SIZE_FOR_UPDATE - (TOTAL_SPACE - usedSpace))) && (totalSize < currentBestFileSize))
    {
        currentBestFileSize = totalSize;
    }
...
void processData()
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            usedSpace = calculateUsedSpace(&rootDir, 0, 0);
            printf("USED SPACE = %d\n",usedSpace);
